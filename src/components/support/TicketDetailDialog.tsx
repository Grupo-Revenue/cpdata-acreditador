...
const statusLabels: Record<string, string> = { pendiente: 'Pendiente', resuelto: 'Resuelto', inactivo: 'Inactivo' };
const priorityLabels: Record<string, string> = { alta: 'Alta', media: 'Media', baja: 'Baja' };
const statusStyles: Record<string, string> = { pendiente: 'bg-warning/10 text-warning border-warning/20', resuelto: 'bg-success/10 text-success border-success/20', inactivo: 'bg-muted text-muted-foreground border-muted' };
const priorityStyles: Record<string, string> = { alta: 'bg-destructive/10 text-destructive border-destructive/20', media: 'bg-warning/10 text-warning border-warning/20', baja: 'bg-muted text-muted-foreground border-muted' };
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>Detalle del ticket de soporte</DialogDescription>
          </DialogHeader>

          <div className="space-y-4">
            <div className="flex gap-2">
              <Badge variant="outline" className={statusStyles[ticket.status]}>{statusLabels[ticket.status]}</Badge>
              <Badge variant="outline" className={priorityStyles[ticket.priority]}>{priorityLabels[ticket.priority]}</Badge>
            </div>
            ...
            {ticket.response_evidence_url && (
              <>
                ...
                <div>
                  ...
                  <Button variant="outline" size="sm" onClick={() => openPreview(ticket.response_evidence_url!, 'Evidencia de respuesta')}>
                    <ExternalLink className="h-4 w-4 mr-2" />
                    Ver / Descargar
                  </Button>
                </div>
